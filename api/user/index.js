...
app.get('/user/homeFeed/:uid',(req,res)=>{
	mysqlConnection.query('SELECT * FROM post where uid = ? OR uid in  (select following_uid from follow WHERE `follower_uid` = ? ) ORDER BY time_stamp desc',[req.params.uid,req.params.uid],(err,rows,fields)=>{
		if(!err)
			res.send(rows);
		else
			console.log(err);
